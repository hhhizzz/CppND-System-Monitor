...
class Util {
 public:
  ...
  static std::vector<std::string> getFirstLine(std::string path);
};
...
void Util::getStream(std::string path, std::ifstream& stream) {
  ...
  if (!stream && !stream.is_open()) {
    stream.close();
    throw std::runtime_error("Non - existing path: " + path);
  }
}

std::vector<std::string> Util::getFirstLine(std::string path) {
  std::string line;
  std::string value;
  float result;
  std::ifstream stream;
